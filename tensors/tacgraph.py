...
class TacGraph:
    ...
    def __init__(self,ops_graph,profile=False):
        ...
        self.testing_by_evd     = ops_graph.testing_by_evd
        self.test_cpt_labels    = ops_graph.test_cpt_labels  # for saving test cpts to file
        
        self.evidence_variables = None # tf variables: tac inputs      
        self.cpt_weights        = None # tf variables: only trainables in TacGraph
        self.test_cpt_weights   = None # tf variables: for test CPTs
        ...
        self.TESTCPTS           = None # computes trainable test cpts
        ...
        if self.testing_by_evd:
            self.__build_tac_graph2(ops_graph)
        else:
    ...
    # Builds TacGraph by compiling callable tf graphs for the tac, metrics and optimizer.
    # assume: ops_graph has testing CPTs directly based on evidence
    def __build_tac_graph2(self,ops_graph):     
        assert not self.finalized
        assert ops_graph.testing_by_evd
        u.show('  Constructing TacGraph: tac...',end='',flush=True)
        ...
        # then in their own tf graph (so we can save them without evaluating tac graph)
        # self.TCPTS() returns trainable cpts (for saving)
        # self.TESTCPTS() returns trainable test cpts (for saving)
        self.TCPTS = self.__trainable_cpts.get_concrete_function(ops_graph) 
        self.TESTCPTS = self.__trainable_test_cpts
        ...
        # only trainable parameters (excluding fixed zero/one parameters)
        weights = self.cpt_weights+self.test_cpt_weights
        self.parameter_count  = sum([tf.size(w).numpy() for w in weights]) 
        self.parameter_count -= self.fixed_01_count
        ...
        assert self.trainable and self.test_cpt_weights
        self.finalized = True
    ...
    def __trainable_test_cpts(self):
        return self.test_cpt_weights
    ...
    def end_training(self,fname):
        ...
        with open(fname,'w') as f:
            ...
            if self.testing_by_evd: # have test cpts 
                test_labels = self.test_cpt_labels
                test_values = tuple(t.numpy() for t in self.TESTCPTS())
                assert len(test_labels) == len(test_values)
                for label, test_cpt in zip(test_labels,test_values):
                    f.write(label)
                    f.write('\n')
                    f.write(np.array2string(test_cpt,formatter=formatter))
                    f.write('\n\n')
    ...
    def save_current_weights(self):
        ...
        weights = self.cpt_weights + self.test_cpt_weights
        self.learned_weights = tuple(w.numpy() for w in weights)
    ...
    def restore_saved_weights(self):
        ...
        weights = self.cpt_weights + self.test_cpt_weights
        for w,v in zip(weights, self.learned_weights):
            w.assign(v)

...
class Op:
    ...
    def __init__(self,inputs,vars):
        ...
        self.vars       = vars   # sorted tuple: var is an abstraction of tbn node
    ...
    def create_variables(ops_graph):
        ...
        test_cpt_weights   = [] # tf trainable variables for test cpt models
        ...
        return tuple(evidence_variables), tuple(cpt_weights), tuple(test_cpt_weights), fixed_01_count
...
class LinearTestCPTOp1(TestCPTOp):
    ...
    def execute(self):
        ...
        value = tf.random.normal(shape,mean=mean,stdev=stddev)
        with tf.name_scope(self.label):
            weight = tf.Variable(initial_value=value,trainable=True,
                        shape=shape,dtype=dtype,name=name) 
        self.cpt_weight = weight
...
class TestCPTOpFactory:
    ...
    def get(self, test_fun=None):
        if test_fun is None:
            test_fun = self.default_test_fun
        if test_fun not in self.factory.keys():
            raise ValueError(f"Test CPT fun {test_fun} is not valid.")
        op = self.factory[test_fun]
        return op

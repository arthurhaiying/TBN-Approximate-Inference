...
class TAC:
    ...
    def __init__(self,tbn,inputs,output,*,hard_inputs=[],trainable=False,
        ...
        self.circuit_type  = 'TAC' if tbn.testing or tbn.testing_by_evd else 'AC'
        self.network_type  = 'TBN' if tbn.testing or tbn.testing_by_evd else 'BN'
    ...
    def __compile(self,net,inputs,output,hard_inputs,trainable,elm_method,elm_wait,profile): 
        ...
        ops_graph = og.OpsGraph(trainable,net.testing,net.testing_by_evd) # empty

        # inference will populate ops_graph with operations that construct tac_graph
        if net1.testing_by_evd:
            inference.trace2(self.output_node,self.input_nodes,net1,jt,ops_graph)
        else:
    ...
    def fit(self,evidence,marginals,loss_type,metric_type,*,batch_size=32):
        ...
        t_loss_record = self.trainer.t_loss_record
        v_metric_record = self.trainer.v_metric_record
        ...
        #u.plot(t_loss_record, "epoch", "Train loss")
        #u.plot(v_metric_record, "epoch", "Val loss")

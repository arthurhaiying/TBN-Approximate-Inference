...
class OpsGraph:
    ...
    def __init__(self,trainable,testing,testing_by_evd):
        ...
        self.testing_by_evd   = testing_by_evd # whether includes testing CPTs directly from evidence
        ...
        self.test_cpt_labels  = []   # for saving learned test CPT model weights
    ...
    def nodes2vars_unsorted(self,nodes,add_batch):
        vars = [self.__get_var(n.id,n.card,n.name) for n in nodes]
        if add_batch:
            var = self.batch_var
            vars.insert(0,var) # [var,*vars]
    ...
    def print_stats(self): 
        mc = pc = mpc = nc = scc = sec = tec = rc = ec = fc = tc = 0
        for op in self.ops:
            op_type = type(op)
            ...
            else: assert op_type in (ops.ScalarOp,)

...
# assume tbn is testing by evidence
def trace2(query_var,evidence_vars,tbn,jt,og):
    print("Compile TBN: testing by evidence")
    assert tbn._for_inference
    assert tbn == jt.tbn
    assert tbn.testing_by_evd
    # the following need to be relaxed
    assert not query_var.has_pruned_values() 
    assert not any(var.has_pruned_values() for var in evidence_vars)
    ...
    # add ops that will create tensors for tested cpts
    for var in qcontext.testing_by_evd_nodes:  # top-down
        __tested_cpt(var,qcontext,jt,og)   # also prunes
...
def __selected_cpt(var,qcontext,jt,og): # var is a tbn node
    ...
    jt.save_sel_cpt_op(var,sel_cpt_op) # cache it, looked up by __cpt_evd()


""" 
    Adds ops that construct a tensor for the tested cpt of tbn node directly based on evidence.
"""
def __tested_cpt(var,qcontext,jt,og):
    assert var.testing_by_evd
...
def __cpt_evd(i,vw,jt,og): # i is a jointree view node (leaf)
    ...
    if var.testing: # cpt op already computed and cached
        ...
        assert cpt_op
    elif var.testing_by_evd:
        cpt_op = jt.lookup_test_cpt_op(var)
        assert cpt_op
    else:    

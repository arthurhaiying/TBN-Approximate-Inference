...
class Jointree:
    ...
    def __init__(self,tbn,elm_order,hard_evd_vars,trainable):
        ...
        self.test_cpt_ops  = {} # maps tbn node to tested cpt
    ...
    """ tested cpt ops """
    def lookup_test_cpt_op(self,var):  # var is a tbn node
        assert var.testing_by_evd
        if var in self.test_cpt_ops:
            return self.test_cpt_ops[var]
        return None

    def save_test_cpt_op(self,var,op): # var is a tbn node
        assert var.testing_by_evd
        self.test_cpt_ops[var] = op

...
class TBN:
    ...
    def __init__(self,name='tbn'):
        ...
        self.testing_by_evd = False # if any node is testing by evidence
    ...
    def add(self,node):
        u.input_check(isinstance(node, Node),
            "{} is not a TBN Node object".format(type(node)))
        ...
        self.testing_by_evd |= node.testing_by_evd
    ...
    def copy_for_inference(self):
        ...
        # for TestNodes, selection evidence need to be replaced with 
        # corresponding nodes in copied net
        if net.testing_by_evd: 
            for n in net.nodes: 
                if n.testing_by_evd:
                    evidence = n.sel_evidence
                    evidence = [net.node(e.name) for e in evidence]
                    n.set_selection_evidence(evidence)
